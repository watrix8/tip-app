...
import { Settings } from 'lucide-react';import { LogOut, AlertCircle, ExternalLink } from 'lucide-react';
import { useState, useEffect } from 'react';
import { initializeStripeConnect, checkStripeAccountStatus } from '@/app/utils/stripeUtils';
import { auth } from '@/app/config/firebase';
import { useAuth } from '@/app/config/AuthContext';
...
// Typ dla propsów z możliwością opcjonalnego currentUser
...
// Dodaj typ FC z propsami
const WaiterPanel: React.FC<WaiterPanelProps> = ({ 
  onLogout, 
  currentUser 
}) => {
  ...
  // Wybierz użytkownika - najpierw z props, potem z kontekstu auth
  ...
  const getTipPageUrl = () => {
    if (!activeUser?.id || !activeUser?.name) return '';
    return `${process.env.NEXT_PUBLIC_BASE_URL}/tip?waiterId=${activeUser.id}&name=${encodeURIComponent(activeUser.name)}`;
  };
  ...
  useEffect(() => {
    async function checkStripeEnabled() {
      if (activeUser?.id) {
        try {
          setIsLoading(true);
          const status = await checkStripeAccountStatus(activeUser.id);
  ...
  const handleStripeSetup = async () => {
    if (activeUser?.id) {
      try {
        await initializeStripeConnect(activeUser.id);
      } catch (error) {
